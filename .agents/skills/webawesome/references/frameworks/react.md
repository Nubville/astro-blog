...
Pro users: get installation instructions from [your workspaces](https://webawesome.com/workspaces) instead.
...
function MyComponent() {
  ...
  return (
    <wa-input value={value} onInput={(event) => setValue((event.target as WaInputElement).value)} />
  );
}
...
function MyComponent() {
  ...
  return (
    <WaInput value={value} onInput={(event) => setValue((event.target as WaInputElement).value)} />
  );
}
